...
# ---------------------------------------------------------------------
# Version history:
#   v1 – Original untouched server (list_events returned text only)
#   v2 – Added event_id to format_events() output
# ---------------------------------------------------------------------
"""
...
def format_events(events) -> str:
    """Format events into a human-readable string (v3.1 with Event ID included)."""
    event_count = len(events) if events else 0
    logger.info(f"Formatting {event_count} events")
    ...
    for event in events:
        ...
        # ⭐ Add the real Google Calendar event ID here
