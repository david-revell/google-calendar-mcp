...
async def call_mcp(tool: str, args: dict):
    ...
    async with stdio_client(server_params) as (read, write):
        ...
        async with ClientSession(read, write) as session:
            ...
            # Performs handshake + advertises server capabilities.
            await session.initialize()

            # Call the resolved tool with the resolved args.
            # This is the actual MCP tool invocation.
...
def interpret_nl(query: str):
    """Send NL text to the agent, parse its JSON tool call, and return tool+args."""
    out = Runner.run_sync(calendar_agent, query)

    # final_output = the model’s last message, containing our JSON
    tool_call = out.final_output

    # The agent returns a JSON string; parse it into a dict
...
# ---------------------------------------------------------------------
def main():
    print("\n=== Calendar MCP Agent (STDIO, v2) ===")

    # Ask the user directly.
    # This is the interactive mode.
    query = input("Ask your calendar anything: ")

    # Step 1: resolve natural language → {tool, args}
    tool, args = interpret_nl(query)

    # The MCP server expects attendees as a single comma-separated string.
    # The agent (correctly) may produce a list of names.
    ...
    # Step 2: actually call the MCP server tool
    events = asyncio.run(call_mcp(tool, args))

    # Step 3: show result raw (as returned from MCP SDK)
    print("\n=== MCP Result ===")
    print(events)
